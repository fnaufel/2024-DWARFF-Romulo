...
Para calcular a correlação entre a lista e o *ranking*, vamos precisar ordenar a lista de alguma forma, pois, se todos os elementos da lista estiverem empatados (i.e., se todos tiverem o mesmo valor de posição), podemos cair em um caso em que o desvio-padrão seja $0$.
...
Eles vão entrar como estando todos empatados no fim da lista, como no exemplo mais abaixo.
...
Uma maneira mais compacta ainda de representar um exemplo.

```{r}
criar_tabela <- function(ranking) {
  
  df <- criar_df(ranking) %>% arrange(pos_ranking)
  
  df %>%
    select(pos_ranking, nome) %>% 
    gt() %>% 
      tab_style(
        cell_fill('green'),
        cells_body(
