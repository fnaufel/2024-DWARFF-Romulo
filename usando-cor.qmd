...
sp1 <- function(um_rk) {
  ...
  (cor(pos_lista, pos_ranking, method = 'spearman') + 1) * k / (2 * p)
...
k1 <- function(um_rk) {
  ...
  k <- um_rk$k
  p <- um_rk$p
  (cor(pos_lista, pos_ranking, method = 'kendall') + 1) * k / (2 * p)
...
g1 <- function(um_rk) {
  ...
  k <- um_rk$k
  p <- um_rk$p
  (rcorr.cens(pos_lista, pos_ranking, outx = TRUE)[[2]] + 1) * k / (2 * p)
...
cov1 <- function(um_rk) {
  ...
  k <- um_rk$k
  p <- um_rk$p
  (cov(pos_lista, pos_ranking, method = 'spearman') + 1) * k / (2 * p)
