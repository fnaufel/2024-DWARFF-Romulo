...
A covariância entre duas variáveis aleatórias $X$ e $Y$ é igual a
...
* Se $x$ e $y$ são pareados:

```{r}
cov_alt_par <- function(x, y) {
  ex <- mean(x)
  ey <- mean(y)
  exy <- mean(x * y)
  exy - ex * ey
...
```{r}
cov_alt_naopar <- function(x, y) {
  ex <- mean(x)
  ey <- mean(y)
  exy <- expand_grid(x = x, y = y) %>% 
    mutate(xy = x * y) %>% 
    pull(xy) %>% 
    mean()
  exy - ex * ey
